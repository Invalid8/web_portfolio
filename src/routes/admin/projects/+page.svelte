...
<div class="p-6 sm:space-y-4 bg-sidebar h-full rounded-md">
	...
	<Tabs.Root value={status}>
		<div class="flex items-center">
			...
			<div class="ml-auto flex items-center gap-2">
				<button
					class={buttonVariants({variant: 'default', size: 'lg', class: 'h-8 gap-1 px-4'})}
		...
		<Tabs.Content value={status}>
				...
				<Table.Root>
					...
					<Table.Body>
						{#each paginatedProjects as project, index}
							<Table.Row
								onclick={() => {
									goto('/admin/projects/view/' + project.id);
								}}
							...
							>
								...
								<Table.Cell
									><Badge class={getStatusClass(project.status)}>{project.status}</Badge
									></Table.Cell
								>
									...
									<DropdownMenu.Root>
										...
										<DropdownMenu.Content align="end" class="bg-background">
											...
											<DropdownMenu.Item
												...
												onclick={() => handleEdit(project.id)}>Edit</DropdownMenu.Item
											>
											<DropdownMenu.Item
			...
			<br />
			{#if paginatedProjects?.length == 0}
				<div class="grid place-content-center place-items-center text-center">
					<SpaceCat />
					<h1 class="font-Bebas text-4xl">Nothing's here buddy</h1>
				</div>
			{/if}
			{#if paginatedProjects?.length > 0}
				<Pagination.Root
